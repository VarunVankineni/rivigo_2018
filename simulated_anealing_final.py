...
def solveForOptBuffer(mastertrip,masterbuffers,usedbuffers,indibuffers,Tmax,maxtime):
    ...
    while((time.time()<temp_time) and (time.time()<maxtime) ):
        base = max(1,7+int(10*(time.time()-temp_time)/(r)))
        T = Tmax / base
        trip = mastertrip.copy()
        buffers = masterbuffers.copy()
        usedbuffers[:] = 0
...
for cases in range(N):
    ...
    Tmax = 1.2*np.max([np.abs(i-Tmean) for i in costlist])
    ...
    wfile.write(currentcase + ' ' + str(snew+matches+2)+'\n')
    wfile.write(str(snew)+' '+str(matches)+'\n')
    for i in range(snew):
         wfile.write(trip.iat[i,0]+' '+trip.iat[i,1]+' '+str(int(trip.iat[i,2]))+'\n')
    ...
    for i in range(matches):
        if(trip.iat[i,-1]=='-1'):
            ...
            ender = trip.iat[i,4]            
        wfile.write(trip.iat[i,1]+' '+trip.iat[i,4]+' '+trip.iat[i,0]+' '+trip.iat[i,-1]+' '+ender+'\n')
